...
const ScheduleViewer = () => {
  ...
  const downloadPDF = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/schedule/download/pdf', {
        method: 'GET',
        headers: {
  ...
  const downloadExcel = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/schedule/download/excel', {
        method: 'GET',
        headers: {
  ...
  const handleReschedule = async (paperId, currentDate) => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/schedule/available-slots?date=${otherDate}`);
      if (!response.ok) {
      ...
      const scheduleResponse = await fetch(`http://localhost:8080/api/schedule/check/${paperId}`);
      ...
      const rescheduleResponse = await fetch(`http://localhost:8080/api/schedule/reschedule/${paperId}`, {
        method: 'POST',
        headers: {
  ...
  const handleSendEmails = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/schedule/send-emails', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': token
  ...
  const handleSendConfirmationEmail = async (paperId) => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/schedule/send-confirmation/${paperId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': token
